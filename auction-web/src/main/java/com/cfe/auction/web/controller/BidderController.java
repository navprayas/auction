...
public class BidderController {
	...
	/*
	 * private List<Integer> getCategoryIdList( List<BidderCategory>
	 * bidderCategoryList) { List<Integer> categoryIds = new
	 * ArrayList<Integer>(); if (bidderCategoryList != null &&
	 * !bidderCategoryList.isEmpty()) { for (BidderCategory category :
	 * bidderCategoryList) { categoryIds.add(category.getCategoryId()); } }
	 * return categoryIds; }
	 */
	...
	public String getActiveMarketList(ModelMap model, HttpSession session) {
		 ...
		 * .getBidderCategory(user.getId(), auctionSearchBean); BidItem bidItem
		 * = AuctionCacheManager .getActiveBidItem(auctionSearchBean); if
		 * (bidItem != null) { List<BidItem> bidItems = new
		 * ArrayList<BidItem>(); bidItems.add(bidItem); List<Integer>
	...
	public String getClosedMarket(ModelMap model, HttpSession session) {
		...
		/*
		 * List<BidderCategory> bidderCategoryList = bidderCategoryService
		 * .getBidderCategory(user.getId(), auctionSearchBean); List<BidItem>
	 ...
	 * session.getAttribute(CommonConstants.USER_NAME); AuctionSearchBean
	 * auctionSearchBean = new AuctionSearchBean(); String userName =
	 * session.getAttribute(CommonConstants.USER_NAME) .toString();
	 * ClientDetails clientDetails = (ClientDetails) session
	 * .getAttribute(SessionConstants.CLIENT_INFO);
	 * auctionSearchBean.setAuctionId(AuctionCacheManager
	 * .getActiveAuctionId(auctionSearchBean));
	 * auctionSearchBean.setSchemaName(clientDetails.getSchemaKey());
	 * List<BidItem> bidItemsList = null; try {
	 * autoBidService.saveAutoBid(userName, categoryId, bidItemId, bidAmount,
	 * "No Comments", auctionSearchBean); } catch (Exception e) {
	 * e.printStackTrace(); } bidItemsList = bidderService.saveAutoBid();
	 ...
	 */
	/*
	 * @RequestMapping(value = "/bid", method = RequestMethod.GET) public
	 * 
	 * @ResponseBody String doBid(
	 * 
	 * @RequestParam(value = "bidItemId", required = true) Long bidItemId,
	 * 
	 ...
	 * ModelMap model, HttpSession session) { System.out.println(bidItemId + " "
	 * + bidType + " " + bidAmount + "" + comments);
	 * session.getAttribute(CommonConstants.USER_NAME); AuctionSearchBean
	 * auctionSearchBean = new AuctionSearchBean(); String userName =
	 * session.getAttribute(CommonConstants.USER_NAME) .toString();
	 * ClientDetails clientDetails = (ClientDetails) session
	 ...
	 * 
	 * auctionSearchBean.setSchemaName(clientDetails.getSchemaKey());
	 * System.out.println(userName); boolean returnVal = false; Bid bid = new
	 * Bid(); bid.setBidId(bidItemId); bid.setBidType(bidType);
	 ...
	 * 
	 * 
	 * auctionSearchBean.setSchemaName("PersistenceUnitB"); Bid bid = new Bid();
	 * bid.setBidId(80L); bid.setBidType(1); bid.setBidAmount(34000.0);
	 * bid.setBidderName("bidder"); bid.setComments("comments");
	 * bid.setAuctionId(80); bid.setBidItemId(211L);
	 * bidsService.createBidService(bid, auctionSearchBean);
	 * 
	 * if (bidType == 2) { BidItem bidItem = null;//
	 * bidItemsCacheService.getBidItem(bidItemId);
	 * 
	 * Map<Long, String> bidItemWithRanks = new HashMap<Long, String>();
	 * Map<Long, String> bidItemWithAutoBidFlag = new HashMap<Long, String>();
	 * Bidder tempBidder = new Bidder(); tempBidder.setBidderName(userName);
	 * List<Bidder> bidders = bidItem.getCurrentBidderList(); int rank =
	 * bidders.indexOf(tempBidder) + 1;
	 * bidItemWithRanks.put(bidItem.getBidItemId(), rank + ""); if
	 * (bidItem.isAutoBidFlag() && rank > 0 && bidders.get(rank -
	 * 1).isAutoBid()) { bidItemWithAutoBidFlag.put(bidItem.getBidItemId(),
	 * "2"); } else { bidItemWithAutoBidFlag.put(bidItem.getBidItemId(), "1"); }
	 ...
	 * returnVal); return "success"; }
	 */
	/*
	 * @RequestMapping(value = "/bidderreport", method = RequestMethod.GET)
	 * public String getwonLists(ModelMap modelMap, HttpSession session) throws
	 * Exception { AuctionSearchBean auctionSearchBean = new
	 * AuctionSearchBean(); User user = (User)
	 * session.getAttribute(SessionConstants.USER_INFO); ClientDetails
	 * clientDetails = (ClientDetails) session
	 * .getAttribute(SessionConstants.CLIENT_INFO);
	 * auctionSearchBean.setSchemaName(clientDetails.getSchemaKey());
	 * LOG.debug("UserName" + user.getUsername());
	 * 
	 * List<BidItem> wonList = bidItemService.getWonList(user.getUsername(),
	 * auctionSearchBean); LOG.debug("closedbids List::" + wonList);
	 * modelMap.addAttribute("wonList", wonList); List<BidderCategory>
	 * categoryList = bidderCategoryService .getAllCategory(AuctionCacheManager
	 * .getActiveAuctionId(auctionSearchBean));
	 * modelMap.addAttribute("bidderCategoryList", categoryList);
	 * LOG.debug(" For category: bidderCategoryList List::" + categoryList);
	 * return "bidderreport"; }
